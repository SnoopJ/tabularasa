...
0. Uninstall any previous version of Tabula Rasa
1. Extract "TabulaRasaV2.2.zip" into the main Starbound folder.
2. Verify that the "tabularasa2.2" folder is in the 'Starbound/mods' directory. If not, you may have extracted the archive to the wrong folder.
...
1. List 'tabularasa' as either an 'include' or 'require' dependency in your .modinfo file.  Use 'include' for optional usage, and 'require' for non-optional dependency. (See "tabularasa2.2/Templates/derivativemod.modinfo.example")
2. Create a folder structure in your mod so it appears like this: "<YOURMODNAMEHERE>/objects/wired/tabularasa/".
2. Copy over the file "tabularasa.object.patch.example" file from the "tabularasa2.2/Templates/" directory to this new directory you made above in step 1.
3. Rename this file "tabularasa.object.patch". Verify that the file path is: "<YOURMODNAMEHERE>/objects/wired/tabularasa/tabularasa.object.patch".
4. Open this "tabularasa.object.patch" in a text editor (Notepad++, etc.).
5. Go to this section of the file: 
	...
	5a. If you want to use a custom artwork for your filter button, copy the example button templates provided in the main "tabularasa2.2" directory, labelled "buttontemplateGREY.png" and "buttontemplateGOLD.png" to your mod directory. 
...
In V2.2, Tabula Rasa has a hard limit of 22 filter buttons that can be displayed in this way, and will notify you how many aren't being shown due to the excess. This hard cap may change in the future when the developers expose more of the UI system to modders.

== COMPATABILITY ==
  
If you would like Tabula Rasa to be -required- to run your mod (as opposed to optional), make sure your .modinfo file lists "tabularasa2" as a dependency.  
See "tabularasa2.2/Templates/" for the "derivativemod.modinfo.example" file for an example.
 
Changes in Starbound mean version 2.2 of Tabula Rasa is NOT backwards compatible with previous versions of the mod.

== CHANGELOG ==
2.2 - Changes to work with Starbound's JSON patching
